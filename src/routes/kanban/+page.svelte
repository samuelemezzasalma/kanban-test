...
import { flip } from 'svelte/animate';
	...
	function dragEnter(laneName, cardIndex) {
		if(laneHover === laneName) {cardHover = cardIndex; }
	}

	function dragStart(event, laneIndex, cardIndex) {
		const draggedElement = document.getElementById(`card-${cardIndex}`);
	  draggedRect = draggedElement.getBoundingClientRect();
		const data = { laneIndex, cardIndex };
		event.dataTransfer.setData('text/plain', JSON.stringify(data));
	}

	function drop(event, laneIndex) {
    ...
    <div id={`lane-${laneIndex}`} class="flex flex-col flex-shrink-0 w-72" animate:flip >
      ...
      <!-- COLUMN CONTAINER -->
        ...
        <div 
				...
				draggable={true} on:dragstart={(event) => dragStart(event, laneIndex, cardIndex)} 
